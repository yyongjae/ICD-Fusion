...
class DataAugmentor(object):
    ...
    def random_local_pyramid_aug(self, data_dict=None, config=None):
        ...
        gt_names, gt_boxes, points = data_dict['gt_names'], data_dict['gt_boxes'], data_dict['points']
        car_indices = np.where(gt_names == 'Car')[0]
        if len(car_indices) > 0:
            gt_boxes_car = gt_boxes[car_indices]

            gt_boxes_car, points, pyramids = augmentor_utils.local_pyramid_dropout(gt_boxes_car, points,
                                                                                       config['DROP_PROB'])
            gt_boxes_car, points, pyramids = augmentor_utils.local_pyramid_sparsify(gt_boxes_car, points,
                                                                                        config['SPARSIFY_PROB'],
                                                                                        config['SPARSIFY_MAX_NUM'],
                                                                                        pyramids)
            gt_boxes_car, points = augmentor_utils.local_pyramid_swap(gt_boxes_car, points,
                                                                          config['SWAP_PROB'],
                                                                          config['SWAP_MAX_NUM'],
                                                                          pyramids)
            gt_boxes[car_indices] = gt_boxes_car
