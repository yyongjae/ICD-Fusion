...
class TransFusionHead(nn.Module):
    ...
    def create_2D_grid(self, x_size, y_size):
        ...
        return coord_base       # （1, y_size*x_size, 2）
    ...
    def predict(self, inputs):
        ...
        lidar_feat = self.shared_conv(inputs)   # (B, C, H, W)

        lidar_feat_flatten = lidar_feat.view(
            batch_size, lidar_feat.shape[1], -1
        )   # (B, C, H*W)
        bev_pos = self.bev_pos.repeat(batch_size, 1, 1).to(lidar_feat.device)    # (B, x_size*y_size, 2)

        # query initialization
        dense_heatmap = self.heatmap_head(lidar_feat)   # (B, num_class, H, W)
        heatmap = dense_heatmap.detach().sigmoid()   # (B, num_class, H, W)
        ...
        )   # (B, num_class, H-nms_kernel_size+1, W-nms_kernel_size+1)
        ...
        heatmap = heatmap * (heatmap == local_max)  # (B, num_class, H, W)
        heatmap = heatmap.view(batch_size, heatmap.shape[1], -1)    # (B, num_class, H*W)

        top_proposals = heatmap.view(batch_size, -1).argsort(dim=-1, descending=True)[
            ..., : self.num_proposals
        ]   # (B, num_proposals)

        top_proposals_class = top_proposals // heatmap.shape[-1]    # (B, proposal_class)
        top_proposals_index = top_proposals % heatmap.shape[-1]     # (B, proposal_index)

        query_feat = lidar_feat_flatten.gather(
            index=top_proposals_index[:, None, :].expand(-1, lidar_feat_flatten.shape[1], -1),
            dim=-1,
        )   # (B, C, num_proposals)
        self.query_labels = top_proposals_class     # (B, proposal_class)

        one_hot = F.one_hot(top_proposals_class, num_classes=self.num_classes).permute(0, 2, 1)
        # (B, num_class, num_proposals)
        ...
        # convert to xy
        query_pos = query_pos.flip(dims=[-1])   # (B, num_proposals, 2)
        bev_pos = bev_pos.flip(dims=[-1])       # (B, x_size*y_size, 2)

        query_feat = self.decoder(
            query_feat, lidar_feat_flatten, query_pos, bev_pos
        )   # (query, key, query_pos, key_pos)
    ...
    def forward(self, batch_dict):
        ...
        # print(res.keys())
        # ['center', 'height', 'dim', 'rot', 'heatmap', 'query_heatmap_score', 'dense_heatmap']

        if not self.training:
            bboxes = self.get_bboxes(res)
            batch_dict['final_box_dicts'] = bboxes
        else:
            gt_boxes = batch_dict['gt_boxes']       # (B,num_boxs,8)
            gt_bboxes_3d = gt_boxes[...,:-1]
            gt_labels_3d =  gt_boxes[...,-1].long() - 1         # (B,num_boxs)
