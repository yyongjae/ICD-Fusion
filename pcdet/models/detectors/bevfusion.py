...
class BevFusion(Detector3DTemplate):
    def __init__(self, model_cfg, num_class, dataset):
        ...
        self.module_topology = [
            ...
            'image_backbone','neck','vtransform','bev_teacher','fuser',
    ...
    def build_bev_teacher(self, model_info_dict):
        if self.model_cfg.get('BEV_TEACHER', None) is None:
            return None, model_info_dict
        bev_teacher = backbones_3d.__all__[self.model_cfg.BACKBONE_3D.NAME](
        ...
        model_info_dict['module_list'].append(bev_teacher)
        model_info_dict['num_point_features'] = bev_teacher.num_point_features
        model_info_dict['backbone_channels'] = bev_teacher.backbone_channels \
            if hasattr(bev_teacher, 'backbone_channels') else None
        return bev_teacher, model_info_dict
